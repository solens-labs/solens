...
export async function listMEden(
    ...
    txIxs.push(await program.instruction.initializeEscrow2(priceBN, bump, {
        accounts: {
            initializer: maker,
...
export async function cancelMEden(
    ...
    txIxs.push(await program.instruction.cancelEscrow({
        accounts: {
            initializer: maker,
...
export async function buyMEden(
    ...
    const makerNftAccount = escrowAccountInfo.tokenAccount;
    let txIxs = []
    txIxs.push(await program.instruction.exchange2(priceBN, mint, {
        accounts: {
            taker: buyer,
            pdaDepositTokenAccount: makerNftAccount,
