...
const magicEden = new anchor.web3.PublicKey('M2mx93ekt1fmXSVkTrUL9xVFHkmME8HTUi5Cyc5aF7K')
const MEdenAutority = new anchor.web3.PublicKey('1BWutmTvYPwDtmw9abTkS4Ssr8no61spGAvW1X6NDix')
const MEAuctionCreator = new anchor.web3.PublicKey('autMW8SgBkVYeBgqYiTuJZnkvDZMVU2MHJh9Jh7CSQ2')
const MEdenFee = new anchor.web3.PublicKey('rFqFJ9g7TGBD8Ed7TPDnvGKZ5pWLPDyxLcvcH2eRCtt')
...
// @ts-ignore
const program = new anchor.Program(MEv2Idl, magicEden, provider)
...
async function getSellerTradeState(seller: anchor.web3.PublicKey, sellerAta: anchor.web3.PublicKey, mint: anchor.web3.PublicKey) {
...
async function getBuyerTradeState(buyer: anchor.web3.PublicKey, mint: anchor.web3.PublicKey) {
    return anchor.web3.PublicKey.findProgramAddress([
        Buffer.from("m2"),
        buyer.toBuffer(),
        MEHouse.toBuffer(),
...
async function depositMEv2(maker: anchor.web3.Keypair, makerNftAccount: anchor.web3.PublicKey, escrowAccount: anchor.web3.PublicKey) {
...
async function withdrawMEv2(maker: anchor.web3.Keypair, makerNftAccount: anchor.web3.PublicKey, escrowAccount: anchor.web3.PublicKey) {
    return program.rpc.cancelEscrow(
        {
            accounts: {
                initializer: maker.publicKey,
...
async function sellMEv2(seller: anchor.web3.Keypair, sellerATA: anchor.web3.PublicKey, tokenAccount: anchor.web3.PublicKey, nftMint: anchor.web3.PublicKey, price: number) {
    const [sellerTradeState, bump] = await getSellerTradeState(seller.publicKey, sellerATA, nftMint)
...
async function cancelSellMEv2(seller: anchor.web3.Keypair, sellerATA: anchor.web3.PublicKey, nftMint: anchor.web3.PublicKey, price: number) {
    const [sellerTradeState, bump] = await getSellerTradeState(seller.publicKey, sellerATA, nftMint)
...
async function buyMEv2(buyer: anchor.web3.Keypair,
    ...
    let [buyerTradeState, bumpp] = await getBuyerTradeState(buyer.publicKey, nftMint)
    let [sellerTradeState, dc] = await getSellerTradeState(seller, sellerATA, nftMint)
...
async function offerMEv2(buyer: anchor.web3.Keypair,
                         seller: anchor.web3.PublicKey,
                         sellerATA: anchor.web3.PublicKey,
                         buyerATA: anchor.web3.PublicKey,
                         nftMint: anchor.web3.PublicKey,
...
async function cancelOfferMEv2(buyer: anchor.web3.Keypair,
                               nftMint: anchor.web3.PublicKey,
                               price: number) {
    let [buyerTradeState, bumpp] = await getBuyerTradeState(buyer.publicKey, nftMint)
    let [userEscrow, bump] = await getUserEscrow(buyer.publicKey)
...
async function main() {
    ...
    */
    let seller = keypair;
    let buyer = keypair1;
    let nftMint = new anchor.web3.PublicKey('6eZEZWSbo7YPxgyk8S3zE9gqJxzgYfL5JHesAJLdcVkV')
    let tokenAccount = new anchor.web3.PublicKey('2bZPBDBVHfJqauscYFNZAHaakkPd6meJC2kayi5sevTb')
    let sellerATA = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, nftMint, seller.publicKey, false)
    let buyerATA = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, nftMint, buyer.publicKey, false)
    let price = 0.2;
    ...
    switch (select) {
        ...
        // @ts-ignore
        case action.Sell:
            let sellTxId = await sellMEv2(seller, sellerATA, sellerATA, nftMint, price)
            console.log(sellTxId)
            break;
        // @ts-ignore
        case action.CancelSell:
            let cancelSellTxId = await cancelSellMEv2(seller, sellerATA, nftMint, price)
            console.log(cancelSellTxId)
            break;
        // @ts-ignore
        case action.Buy:
            let buyTxId = await buyMEv2(buyer, seller.publicKey, sellerATA, buyerATA, nftMint, price);
            console.log(buyTxId)
            break;
        // @ts-ignore
        case action.Offer:
            price = 0.1
            let offerTxId = await offerMEv2(buyer, seller.publicKey, sellerATA, buyerATA, nftMint, price)
        ...
        case action.AcceptOffer:
            price = 0.1
            let acceptOfferTxId = await acceptOfferMEv2(seller, buyer.publicKey, sellerATA, sellerATA, buyerATA, nftMint, price)
